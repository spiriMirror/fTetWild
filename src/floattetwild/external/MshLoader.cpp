...
#include <floattetwild/Exception.h>
    ...
    if (m_binary) {
        ...
        if (one != 1) {
            std::cerr << "Warning: binary msh file " << filename
                      << " is saved with different endianness than this machine." << std::endl;
            throw NOT_IMPLEMENTED;
        }
    }
...
MshLoader::FieldNames MshLoader::get_element_field_names() const
{
    ...
    for (FieldMap::const_iterator itr = m_element_fields.begin(); itr != m_element_fields.end();
         itr++) {
        result.push_back(itr->first);
    }
    return result;
    ...
    if (m_binary) {
        ...
        for (size_t i = 0; i < num_nodes; i++) {
            ...
            m_nodes[node_idx * 3 + 1] =
              *reinterpret_cast<Float*>(&data[i * (4 + 3 * m_data_size) + 4 + m_data_size]);
            m_nodes[node_idx * 3 + 2] =
              *reinterpret_cast<Float*>(&data[i * (4 + 3 * m_data_size) + 4 + 2 * m_data_size]);
        }
        ...
        for (size_t i = 0; i < num_nodes; i++) {
            fin >> node_idx;
            node_idx -= 1;
            fin >> m_nodes[node_idx * 3] >> m_nodes[node_idx * 3 + 1] >> m_nodes[node_idx * 3 + 2];
        }
    }
...
void MshLoader::parse_elements(std::ifstream& fin)
{
    ...
    auto copy_to_array = [&](const std::vector<int>& elements, const int nodes_per_element) {
...
void MshLoader::parse_unknown_field(std::ifstream& fin, const std::string& fieldname)
{
    std::cerr << "Warning: \"" << fieldname << "\" not supported yet.  Ignored." << std::endl;
    std::string endmark =
      fieldname.substr(0, 1) + "End" + fieldname.substr(1, fieldname.size() - 1);
