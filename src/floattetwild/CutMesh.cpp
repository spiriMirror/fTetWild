            ...
            for (int j = 0; j < 4; j++) {
                ...
                if (map_v_ids.find(v_id) == map_v_ids.end()) {
                    ...
                    if (ori != Predicates::ORI_ZERO &&
                        ...
                        is_snapped.push_back(1);
                    else
                        is_snapped.push_back(0);
...
{
    is_projected.resize(v_ids.size(), 0);
...
void floatTetWild::CutMesh::revert_totally_snapped_tets(int a, int b)
{
    ...
    for (int i = a; i < b; i++) {
        ...
        if (is_v_on_plane(t[0]) && is_v_on_plane(t[1]) && is_v_on_plane(t[2]) &&
            ...
            for (int j = 0; j < 3; j++) {
                if (is_snapped[tmp_t[j]] == 1) {
                    is_snapped[tmp_t[j]] = 0;
                    break;
                }
            }
        }
  ...
  std::vector<int>&                  subdivide_t_ids)
{
    ...
    for (int i = 0; i < edges.size(); i++) {
        ...
        if (!is_result) {
            ...
            logger().error("to_plane_dists: {}, {}", to_plane_dists[e[0]], to_plane_dists[e[1]]);
            logger().error("plane distances: {}, {}",
                           get_to_plane_dist(mesh.tet_vertices[v1_id].pos),
                           get_to_plane_dist(mesh.tet_vertices[v2_id].pos));
            logger().error("e[0] = {}", e[0]);
            // fortest
            return false;
        }
...
bool floatTetWild::CutMesh::check()
{
    ...
    for (auto& m : map_v_ids) {
        ...
        if (std::fabs(dist) < mesh.params.eps_coplanar && to_plane_dists[lv_id] != 0 &&
            ...
            logger().error("to_plane_dists[lv_id]: {}", to_plane_dists[lv_id]);
            logger().error("is_snapped[lv_id]: {}", is_snapped[lv_id]);
            is_good = false;
        }
    }
    return is_good;
