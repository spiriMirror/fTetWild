...
void random_shuffle(BeginIt begin, EndIt end)
{
...
{  /// use 38416, 232368 as example //todo: think why
    ...
    random_shuffle(sorted_f_ids.begin(), sorted_f_ids.end());
  ...
  bool                                          is_again)
{
    ...
    if (cut_t_ids.empty()) {
        logger().info("get_area = {}", get_area(vs[0], vs[1], vs[2]));

        logger().info("f {} : {} {} {}",
                      insert_f_id,
                      input_faces[insert_f_id][0],
                      input_faces[insert_f_id][1],
                      input_faces[insert_f_id][2]);
    ...
    if (!cut_mesh.get_intersecting_edges_and_points(
        ...
        logger().error("FAIL get_intersecting_edges_and_points");
        return false;
    }
    ...
    if (!subdivide_tets(insert_f_id,
        ...
        logger().error("FAIL subdivide_tets");
        return false;
    }
    ...
    for (int I = 0; I < b_edge_infos.size(); I++) {
        ...
        if (!subdivide_tets(-1,
            ...
            if (!is_inside_envelope) {
                ...
                logger().error("SEMI-FAIL subdivide_tets");
            }
  ...
  const std::vector<int>&                       sorted_f_ids)
{
    ...
    for (int i = 0; i < sorted_f_ids.size(); i++) {
        ...
        if (covered_fs_infos[f_id].empty()) {
            logger().error("covered_fs_infos[f_id].empty()!!!");
            pausee();
            continue;
        }
        ...
        for (const auto& p : ps) {
            ...
            if (!is_inside) {
                logger().info("check is_input_face_covered fail!! f_id = {}, i = {}", f_id, i);
                pausee();
                break;
            }
        }
