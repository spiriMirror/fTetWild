...
void floatTetWild::init(Mesh& mesh, AABBWrapper& tree)
{
    ...
    if (mesh.params.log_level < 3) {
        if (!mesh.params.output_path.empty()) {
            auto abs_path = std::filesystem::absolute(mesh.params.output_path + "_" +
                                                      mesh.params.postfix + "_cutting");
            logger().info("Cutting outputting to {}", abs_path.string());
            output_surface(mesh, abs_path.string());
        }
...
void floatTetWild::output_info(Mesh& mesh, const AABBWrapper& tree)
{
    ...
    if (mesh.params.log_level > 1) {
        if (!mesh.params.output_path.empty()) {
            auto abs_path = std::filesystem::absolute(mesh.params.output_path + "_" +
                                                      mesh.params.postfix + "_opt");
            logger().info("Optimization outputting to {}", abs_path.string());
    ...
    for (int i = 0; i < tets.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) {
            if (tet_vertices[tets[i][j]].is_removed) {
                logger().info("tet_vertices[tets[{}][{}]].is_removed", i, j);
            }
        }
    }
    for (int i = 0; i < tets.size(); i++) {
        ...
        if (is_inverted(mesh, i)) {
            logger().info(
              "tet {} inverted: {} {} {} {}", i, tets[i][0], tets[i][1], tets[i][2], tets[i][3]);
        }
    }
    ...
    for (int i = 0; i < tet_vertices.size(); i++) {
        ...
        if (conn_tets != tmp_conn_tets[i]) {
            logger().error("conn_tets error");
            std::string conn_tets_str = "";
            for (auto& ii : tet_vertices[i].conn_tets)
                conn_tets_str += std::to_string(ii) + " ";
            logger().error("actual: {}", conn_tets_str);
            conn_tets_str = "";
            for (auto& ii : tmp_conn_tets[i])
                conn_tets_str += std::to_string(ii) + " ";
            logger().error("expected: {}", conn_tets_str);
        }
    }
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (tets[i].is_bbox_fs[j] != NOT_BBOX) {
                ...
                if (get_opp_t_id(mesh, i, j) != OPP_T_ID_BOUNDARY) {
                    logger().error("wrong-marked bbox face");
                }
            }
            else {
                if (get_opp_t_id(mesh, i, j) == OPP_T_ID_BOUNDARY) {
                    logger().error("unmarked bbox face");
                }
            }
        }
    }
    ...
    for (int i = 0; i < tet_vertices.size(); i++) {
        ...
        if (tet_vertices[i].is_on_bbox && tet_vertices[i].is_on_surface) {
            logger().error("error tet_vertices[i].is_on_bbox && tet_vertices[i].is_on_surface");
            logger().error("v {}", i);
        }
    ...
    if (!mesh.params.output_path.empty()) {
        {
            auto opt_file_name = mesh.params.output_path + "_" + mesh.params.postfix + "_opt.info";
            auto abs_file_name = std::filesystem::absolute(opt_file_name);
            logger().info("Detailed info outputting to {}", abs_file_name.string());
            output_surface(mesh, opt_file_name);
        }
        {
            auto xyz_file_name = mesh.params.output_path + "_" + mesh.params.postfix + "_b_vs.xyz";
            auto abs_xyz_file_name = std::filesystem::absolute(xyz_file_name);
            logger().info("Boundary vertices outputting to {}", abs_xyz_file_name.string());
            std::ofstream fout(abs_xyz_file_name);
            for (auto& v : mesh.tet_vertices) {
...
void floatTetWild::check_envelope(Mesh& mesh, const AABBWrapper& tree)
{
    ...
    for (auto& v : mesh.tet_vertices) {
        ...
        if (d > mesh.params.eps_2) {
            ...
            logger().info("d = {}, eps_input = {}", d, check_eps);
        }
    }
...
void floatTetWild::apply_sizingfield(Mesh& mesh, AABBWrapper& tree)
{
    ...
    for (int i = 0; i < 20; i++) {
        ...
        logger().info("iteration {} max_energy {}", i, max_energy);
...
#define SF_CONDITION t.is_surface_fs[j] <= 0 && t.surface_tags[j] == c_id
    ...
    if (!mesh.params.output_path.empty()) {
        auto abs_file_name = std::filesystem::absolute(
          mesh.params.output_path + "_" + mesh.params.postfix + "_tracked_surface.obj");
        logger().info("Tracked surface outputting to {}", abs_file_name.string());
        igl::writeSTL(abs_file_name.string(), V_sf, F_sf);
    }
#undef SF_CONDITION
}
...
void floatTetWild::untangle(Mesh& mesh)
{
    ...
    logger().info("fixed {} tangled elements", cnt);
}
...
void floatTetWild::smooth_open_boundary_aux(Mesh& mesh, const AABBWrapper& tree)
{
    ...
    for (int it = 0; it < IT; it++) {
        ...
        logger().debug("smoothed {}/{} vertices", cnt_s, cnt);
...
void floatTetWild::manifold_edges(Mesh& mesh)
{
    ...
    while (!edge_queue.empty()) {
        ...
        logger().info("groups {}/{}", tet_groups.size(), n_t_ids.size());
        logger().debug("edge {} {}", e[0], e[1]);
        for (int t_id : n_t_ids) {
            logger().debug("tet {}: print info", t_id);
            tets[t_id].print();
        }
...
void floatTetWild::manifold_surface(Mesh& mesh, Eigen::MatrixXd& V, Eigen::MatrixXi& F)
{
    ...
    for (int v_id = 0; v_id < V_size; v_id++) {
        ...
        if (f_group.size() == conn_f4v[v_id].size())
            continue;
        //
        logger().debug("manifold surface processing");
    ...
    logger().debug("non-manifold vertices count: {}", cnt);
}
